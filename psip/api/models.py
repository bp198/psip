...
class FADRequest(BaseModel):
    ...
    sigma_u: float = Field(
        ..., gt=0, description="Ultimate tensile strength (MPa). Typical X65: 531 MPa."
    )
    K_mat: float = Field(
        ..., gt=0, description="Fracture toughness (MPa·√m). Typical range: 80–200."
    )
    E: float = Field(
        207_000.0, gt=0, description="Young's modulus (MPa). Default: 207,000 (carbon steel)."
    )

    # Pipe geometry
    outer_diameter: float = Field(
        ..., gt=0, description="Pipe outer diameter (mm). E.g., 914 mm (36 inch)."
    )
    wall_thickness: float = Field(..., gt=0, description="Pipe wall thickness (mm). E.g., 14.3 mm.")
    pressure: float = Field(
        ..., gt=0, description="Operating internal pressure (MPa). E.g., 7.5 MPa."
    )

    # Flaw geometry
    flaw_depth: float = Field(
        ..., gt=0, description="Flaw depth *a* (mm) — through-thickness direction."
    )
    ...
    scf: float = Field(
        1.0, gt=0, description="Stress concentration factor at weld toe. Default: 1.0."
    )
...
class FADResponse(BaseModel):
    ...
    is_acceptable: bool = Field(
        ..., description="True if flaw lies inside the FAD envelope (safe)."
    )
    ...
    reserve_factor: float = Field(
        ...,
        description=(
            "Safety margin: ratio of distance to FAD curve vs distance to assessment point. "
            "> 1.0 = safe."
        ),
    )
    assessment_point: Dict[str, float] = Field(
        ..., description="The (Lr, Kr) point plotted on the FAD."
    )
...
class MCRequest(BaseModel):
    ...
    n_simulations: int = Field(
        10_000, ge=100, le=100_000, description="Number of Monte Carlo trials. Range: 100–100,000."
    )
    ...
    random_seed: Optional[int] = Field(
        42, description="Random seed for reproducibility. None = random."
    )
...
class MCResponse(BaseModel):
    ...
    n_failures: int = Field(
        ..., description="Number of trials where the flaw was assessed as unacceptable."
    )
    P_f: float = Field(
        ..., description="Point estimate of failure probability P_f = n_failures / n_simulations."
    )
    P_f_lower: float = Field(
        ..., description="Lower bound of 95% Wilson confidence interval on P_f."
    )
    P_f_upper: float = Field(
        ..., description="Upper bound of 95% Wilson confidence interval on P_f."
    )
    ...
    mean_reserve: float = Field(
        ..., description="Mean reserve factor for simulations inside the FAD envelope."
    )
    risk_level: str = Field(
        ..., description="Qualitative risk band: 'LOW' (<0.1), 'MEDIUM' (0.1–0.5), 'HIGH' (>0.5)."
    )
...
class AttackerPrior(BaseModel):
    ...
    attacker_type: str = Field(
        ..., description="Attacker type: 'strategic', 'opportunistic', or 'state_actor'."
    )
    prior: float = Field(
        ..., ge=0, le=1, description="Prior probability. All priors must sum to 1.0."
    )
...
class GameRequest(BaseModel):
    ...
    budget: float = Field(
        ...
        le=1.0,
        description=(
            "Defender coverage budget B ∈ (0, 1]. "
            "Fraction of the network that can be actively defended."
        ),
    )
    n_nodes: int = Field(
        20,
        ...
        le=50,
        description=(
            "Number of nodes in the synthetic test network. Default: 20 (Gulf Coast model)."
        ),
    )
...
class GameResponse(BaseModel):
    ...
    coverage_effectiveness: float = Field(
        ..., description="Expected loss reduction vs. zero coverage (%)."
    )
    coverage_by_segment: Dict[str, float] = Field(
        ..., description="Coverage allocation per segment: {segment_id: c_i*}."
    )
    attacker_strategy: Dict[str, float] = Field(
        ..., description="Attacker mixed strategy at equilibrium: {segment_id: prob}."
    )
    top_3_defended: List[str] = Field(
        ..., description="The three segments receiving the highest coverage allocation."
    )
...
class AdversarialRequest(BaseModel):
    ...
    n_steps: int = Field(
        40, ge=1, le=200, description="Iteration count for BIM/PGD. Ignored for FGSM."
    )
    n_samples: int = Field(200, ge=10, le=2000, description="Number of NDE test samples to attack.")
    ...
    scf: float = Field(
        1.5,
        gt=0,
        description="Weld SCF used for physics-scaled ε. Only used if physics_scaled=True.",
    )
...
class AdversarialResponse(BaseModel):
    ...
    epsilon_effective: float = Field(
        ..., description="Effective ε after physics scaling (= ε_req if not scaled)."
    )
    ...
    adversarial_accuracy: float = Field(
        ..., description="Model accuracy on adversarial inputs (%). Lower = stronger attack."
    )
    attack_success_rate: float = Field(
        ..., description="Fraction of correctly-classified clean samples flipped by the attack (%)."
    )
    ...
    class_breakdown: Dict[str, float] = Field(
        ..., description="Attack success rate per defect class: {class_name: asr%}."
    )
